...
def search_res_base(adata_base, n_setting, min_res = 1e-3, max_res = 5, 
    ...
    while this_step < max_step:
        ...
        sc.tl.leiden(adata_base, resolution = this_resolution, random_state = random_state)
        res = adata_base.obs["leiden"].astype(int)
    ...
    label = adata_base.obs['leiden']
    return label
            
    


def run_leiden(adata_base_methods, n_setting = 7, neigh = 50, use_rep = 'STAGATE_Fusion'):
