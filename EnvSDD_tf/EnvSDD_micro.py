...
class STAGATE():
    ...
    def __init__(self, hidden_dims, alpha, n_epochs=500, lr=0.0001,
                 ...
                 verbose=True, random_seed=2020, fusion_lambda=1.0):
        ...
        # 注意：增加 Beta
        self.loss, self.H, self.C, self.ReX = self.gate(self.A, self.prune_A, self.Beta, self.X)
    ...
    def build_placeholders(self):
        ...
        self.Beta = tf.sparse_placeholder(dtype=tf.float32)  # 新增：微环境先验 β
    ...
    def __call__(self, A, prune_A, Beta, X):
        for epoch in tqdm(range(self.n_epochs), disable=not self.verbose):
            self.run_epoch(epoch, A, prune_A, Beta, X)

    def run_epoch(self, epoch, A, prune_A, Beta, X):
    ...
    def infer(self, A, prune_A, Beta, X):
    ...
    def Conbine_Atten_l(self, input):
        if self.alpha == 0:
            return [sp.coo_matrix((input[layer][1],
                                   (input[layer][0][:, 0], input[layer][0][:, 1])),
                                  shape=(input[layer][2][0], input[layer][2][1])) for layer in input]
        else:
            Att_C = [sp.coo_matrix((input['C'][layer][1],
                                    (input['C'][layer][0][:, 0], input['C'][layer][0][:, 1])),
                                   shape=(input['C'][layer][2][0], input['C'][layer][2][1])) for layer in input['C']]
            Att_pruneC = [sp.coo_matrix((input['prune_C'][layer][1],
                                         (input['prune_C'][layer][0][:, 0], input['prune_C'][layer][0][:, 1])),
                                        shape=(input['prune_C'][layer][2][0], input['prune_C'][layer][2][1])) for layer in input['prune_C']]
            return [self.alpha * Att_pruneC[layer] + (1 - self.alpha) * Att_C[layer]
                    for layer in input['C']]
